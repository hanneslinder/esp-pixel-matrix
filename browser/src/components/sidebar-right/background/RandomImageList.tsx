...
import { getApiKeyFromLocalStorage, saveApiKeyToLocalStorage } from "../../../utils/storage";
...
export const RandomImageList: React.FC<Props> = ({ imageSelected }) => {
	...
	const initialApiKey: string | null = config.unsplashApiKey === "" ? getApiKeyFromLocalStorage() : config.unsplashApiKey;
	...
	const loadImages = async () => {
		...
		try {
			const response = await fetch(`https://api.unsplash.com/photos/random?client_id=${apiKey}&orientation=landscape&count=10`);
	...
	const renderApiInput = () => {
		return (
			<div className="api-key-input">
				{apiError && <div className="api-error-msg">The unsplash API returned an error.</div>}
				<div>
					Please enter your{" "}
				...
				<input type="text" onChange={(e) => setNewApiKey(e.target.value)} />
