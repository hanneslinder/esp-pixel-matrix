...
import { AppStateToPersist, deleteItem, SavedItem, saveView } from "../../../utils/storage";
import { Canvas, PixelData } from "../../canvas/Canvas";
import { SvgIcon } from "../../utils/SvgIcon";

const iconSave = require("../../../assets/file-arrow-down.svg");
const iconNew = require("../../../assets/cloud-plus.svg");
const iconDelete = require("../../../assets/trash.svg");

// sample some pixels, should be unique enough
function getItemKey(item: SavedItem) {
	return item.id + item.pixelData[32 * 5].c + item.pixelData[32 * 15].c + item.pixelData[15 * 10].c;
}
...
export const SaveLoad: React.FC<SavedItemProps> = view(({ getCanvas }) => {
	...
	const savedItems = appState.savedItems.sort((a, b) => (a.modified < b.modified ? 1 : -1));
				...
				<div className="saved-item-list">
					{savedItems.map((item) => (
						<div key={getItemKey(item)} className="saved-item">
							...
							<PreviewCanvas pixelData={item.pixelData} onClick={() => loadItem(item)} />
							<SvgIcon icon={iconSave} className="saved-item-overwrite" title="overwrite" onClick={() => save(item.name, item.id)} />
							<SvgIcon icon={iconDelete} className="saved-item-delete" title="delete" onClick={() => deleteItem(item.id)} />
						</div>
					))}
				</div>
			</div>
...
class PreviewCanvas extends React.Component<PreviewCanvasProps, {}> {
	...
	render() {
		return <canvas ref={(r) => (this.canvasRef = r)} onClick={this.props.onClick} />;
	}
}
