				...
				if (letterPixel) {
					...
					for (let pixelX = 0; pixelX < textSize; pixelX++) {
						for (let pixelY = 0; pixelY < textSize; pixelY++) {
							const px =
								posX * appState.matrix.pixelRatio +
								(x + offsetX) * textSize * appState.matrix.pixelRatio +
								l * 6 * textSize * appState.matrix.pixelRatio +
								1 +
								appState.matrix.pixelRatio * pixelX;
							const py =
								posY * appState.matrix.pixelRatio + y * textSize * appState.matrix.pixelRatio + 1 + appState.matrix.pixelRatio * pixelY;
							ctx.fillRect(px, py, appState.matrix.pixelRatio - 1, appState.matrix.pixelRatio - 1);
						}
					}
				} else {
