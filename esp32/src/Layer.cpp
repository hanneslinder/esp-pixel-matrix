...
// /**
//  * Experimental layer class to do play with pixel in an off-screen buffer before painting to the
//  DMA
//  *
//  * Requires FastLED
//  *
...
// void Layer::drawPixel(int16_t x, int16_t y, int r, int g, int b) { drawPixel(x, y, CRGB(r, g,
// b)); }
...
//      indicates the baseline — the bottom-most row — of subsequent text. Characters may vary in
//      size and width, and don’t necessarily begin at the exact cursor column (as in below, this
//      character starts one pixel left of the cursor, but others may be on or to the right of it).
//                    */
...
//     }
//   } else // custom font
//   /* As we can't reliable know what is the actual FIRST and last 'lit' pixel, we need to check
//   what
...
//       {
//         if (writeBackToBg) // write the foreground to the background layer... perhaps so we can
//         do
//                            // stuff later with the _fgLayer.
//           _bgLayer.pixels->data[y][x] = _bgLayer.pixels->data[y][x];
//         else
//           disp->drawPixelRGB888(x, y, _bgLayer.pixels->data[y][x].r,
...
//         else
//           disp->drawPixelRGB888(x, y, _fgLayer.pixels->data[y][x].r,
...
// void Blend(MatrixPanel_I2S_DMA* disp, const Layer& _bgLayer, const Layer& _fgLayer, uint8_t
// ratio)
// {
//   CRGB _pixel = 0;
